...
let pAequorFactory = (specimenNum, dna) => {
  return {
    ...
    mutate() { // method that changes 1 random base
      ...
      while (random === dna[index]) { // loop until the new random base is different from the old base
    ...
    compareDNA(pAequorObj) { // method to compare the current organism with the previous one
      let counter = 0;
    ...
    willLikelySurvive() { // returns true if at least 60% of its bases are C or G
      ...
      for (let i = 0; i < this.dna.length; i++) { // looping over bases of the caller object
        if (this.dna[i] == "C" || this.dna[i] == "G") { // if the base is C or G...
          counter += 1; // ...increase counter
        }
      }
...
// method to create 30 specimens that can survive (at least 60% of the DNA strand must be C's or G's)
let pool = () => {
  let sampleArray = []; // array to contain specimens for which .willLikelySurvive() returns "true"  
  while (sampleArray.length < 30) { // loop until the adaptingSpecimen array contains 30 elements
...
// creating a pool of 30 surviving specimens
